...
export function apply(ctx, config) {
  ...
  try {
    ctx.systemPrompt.section({
      ...
      text: [
        ...
        '3. 用 unity_pool_bind(instance="Name@hash" 或 hash 前缀) 把本会话目标实例锁定为指定实例（一个会话只锁定一个实例）。',
  ...
  try {
    ...
    ctx.tools.register(defineTool({
      ...
      description: '把本会话的目标实例锁定为一个 Unity 实例（一个会话只锁定一个实例）。传 instance（Name@hash 或 hash 前缀）锁定指定实例；传 serviceId 则锁定该服务上第一个实例；都不传则自动分配一个未被其他会话锁定的实例。enforceExclusive 下同一实例默认不能被第二个会话锁定，确有需要可传 force=true。返回锁定结果与状态视图。',
      parameters: {
      ...
      execute(args, exec) {
        const sessionId = requireSession(exec)
        const a = args || {}
        const result = pool.bind(sessionId, { instance: a.instance, serviceId: a.serviceId, force: Boolean(a.force) })
    ...
    ctx.tools.register(defineTool({
      name: 'unity_mcp',
      description: '代理调用本会话目标实例上的 MCP 工具（先通过本插件）：调用前自动确保本会话的 MCP 会话激活到目标实例（per MCP-Session-Id 隔离，其他会话用其他实例互不干扰），然后转发 tools/call 并返回结果。tool 为 mcp-for-unity 工具名（如 manage_scene、manage_gameobject、manage_camera、read_console 等）；params 为该工具参数对象。本工具前需先用 unity_pool_bind 锁定目标实例。',
      parameters: {
        ...
        handler: (req, res) => {
          ...
          if (req.method === 'POST' && (p === '/unity-pool/api/bind' || p === '/unity-pool/api/bind/')) {
            readJson(req).then(body => {
              ...
              try {
                const result = pool.bind(sessionId, { instance: body.instance, serviceId: body.serviceId, force: Boolean(body.force) })
