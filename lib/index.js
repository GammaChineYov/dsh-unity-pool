  ...
  try {
    ctx.systemPrompt.section({
      ...
      text: [
        ...
        '3. 用 unity_pool_bind(instance="Name@hash" 或 hash 前缀) 把本会话目标实例锁定为指定实例（一个会话只锁定一个实例）；首次绑定或切换到另一服务时，返回结果附带该服务的 MCP 工具列表（tools 字段，含名称/描述/参数 schema），可据此直接 unity_mcp 调用。',
