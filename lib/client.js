...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var S = {
      ...
      dockWrap: { display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', fontSize: 10, color: 'var(--dsw-alias-label-secondary,#999)', padding: '1px 0', boxSizing: 'border-box', width: 'calc(100% - var(--dsh-composer-side-clearance) - var(--dsh-composer-side-clearance) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset) - var(--dsh-composer-dock-inset))', maxWidth: 'calc(var(--dsh-composer-card-max-width) - 4 * var(--dsh-composer-dock-inset))', margin: '0 auto' },
      ...
      dockUnbound: { opacity: 0.65 }, // v0.4.2：未绑定 Unity 时开关可切（预配置），仅灰显提示，不挡点击
      // v0.5.3：dock 内的绑定入口（新会话没有会话头部槽 → 头部胶囊不渲染，绑定入口必须落在输入框上方）
      dockBindWrap: { position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 4, flex: '0 0 auto' },
      dockBindBtn: { display: 'inline-flex', alignItems: 'center', gap: 4, font: 'inherit', fontSize: 10, lineHeight: 1.7, padding: '0 6px', borderRadius: 999, border: '1px solid #888888', background: 'transparent', color: 'var(--dsw-alias-label-secondary,#bbb)', cursor: 'pointer', whiteSpace: 'nowrap' },
      dockBindBtnOn: { borderColor: '#22c55e', color: 'var(--dsw-alias-label-primary,inherit)' },
      dockPop: { position: 'absolute', bottom: '100%', left: 0, marginBottom: 6, zIndex: 2000, minWidth: 280, maxWidth: '92vw', maxHeight: '50vh', overflow: 'auto', background: 'var(--dsw-alias-bg-layer-1,#1f1f1f)', border: '1px solid var(--dsw-alias-border-l1,#88888866)', borderRadius: 10, boxShadow: '0 8px 28px rgba(0,0,0,.4)', padding: '8px 10px', font: 'inherit', fontSize: 11, color: 'var(--dsw-alias-label-primary,inherit)', textAlign: 'left', cursor: 'default' },
    }
    ...
    function UnityPoolUtility(props) {
      ...
      function lock(instanceId) {
        // v0.5.3：并行开发同一实例是正常用法 → 直接按默认（force=true）绑定。
        // 旧实现先发 force=false 试探、再按错误文案 /已被会话/ 自动重试；v0.5.1 改了拒绝文案后
        // 这个匹配失效 → 用户点「锁定」只看到 enforceExclusive 报错、根本绑不上（本次修复的实锤 bug）。
        post('/unity-pool/api/bind', { instance: instanceId })
      }
    ...
    var STATE_SWITCHES = [
      ...
      { key: 'stateConsoleError', label: 'Console 错误' },
    ]
    ...
    function UnityPoolStateDock(props) {
      ...
      // v0.5.3 绑定入口下沉到 dock：新会话（首条消息之前）没有会话头部槽 → 头部「Unity」胶囊
      // 根本不渲染，导致「无法提前绑定实例」；dock 槽在新会话就有，故把服务池/实例列表与
      // 锁定/解绑/扫描搬到这里（头部胶囊保持不变，两处经共享 store 同步）。
      var [pool, setPool] = useState({ open: false, view: null, error: null, busy: false })
      var loadPool = useCallback(function () {
        if (!sessionId) return Promise.resolve(null)
        return fetch(apiUrl('/unity-pool/api/status', sessionId))
          .then(function (r) { return r.json() })
          .then(function (res) {
            if (!res || res.ok !== true) return null
            setPool(function (p) { return { ...p, view: res.value } })
            storeSet(sessionId, undefined, extractBinding(res.value))
            return res.value
          })
          .catch(function () { return null })
      }, [sessionId])
      useEffect(function () { if (pool.open) loadPool() }, [pool.open, loadPool])
      // 点浮窗外收起（浮窗与按钮内部都 stopPropagation，不会自关）
      useEffect(function () {
        if (!pool.open || typeof document === 'undefined') return
        function onDoc() { setPool(function (p) { return { ...p, open: false } }) }
        document.addEventListener('click', onDoc)
        return function () { document.removeEventListener('click', onDoc) }
      }, [pool.open])
      function poolPost(path, body) {
        if (!sessionId) return
        setPool(function (p) { return { ...p, busy: true, error: null } })
        postJson(path, Object.assign({ sessionId: sessionId }, body || {})).then(function (res) {
          if (res && res.ok === true) {
            var view = res.value.view || res.value
            storeSet(sessionId, (view && view.state) || undefined, extractBinding(view))
            setPool(function (p) { return { ...p, busy: false, view: view, error: null } })
          } else {
            setPool(function (p) { return { ...p, busy: false, error: (res && res.error && res.error.message) || '操作失败' } })
          }
        }).catch(function (err) {
          setPool(function (p) { return { ...p, busy: false, error: String((err && err.message) || err) } })
        })
      }
      // 并行开发同一实例是正常用法 → 直接默认绑定（force 由服务端默认 true，不再先试探后重试）
      function dockBind(instanceId) { poolPost('/unity-pool/api/bind', { instance: instanceId }) }
      function dockUnbind() { poolPost('/unity-pool/api/unbind', {}) }
      function dockScan() { poolPost('/unity-pool/api/scan', {}) }
      ...
      // v0.4.2：开关按会话独立 + 未绑定可切（预配置，绑定后生效）——不再 disabled:!bound；
      // 子项仅在总开关关时禁用（总开关 gate 所有子项）；未绑定时灰样式提示但可点。
      var subDisabled = !enabled
      var chipCells = STATE_SWITCHES.map(function (sw) {
        ...
        var style = Object.assign({}, S.dockChip, on ? S.dockChipOn : null, subDisabled ? S.dockDisabled : null, !bound ? S.dockUnbound : null)
        return h('button', {
          key: sw.key,
          type: 'button',
          style: style,
          title: sw.label + (on ? '（开）' : '（关）') + (bound ? '' : '，未绑定 Unity，绑定后生效'),
          disabled: !enabled,
          onClick: function () { switchIt(sw.key, !on) },
        }, short)
      })

      // v0.5.3：dock 内的绑定入口（点开 → 服务池实例列表 → 锁定/解绑/扫描）
      var bindName = (stVal.binding && stVal.binding.instanceName) || null
      var bindAlive = stVal.binding ? stVal.binding.alive !== false : false
      var bindEl = h('span', { style: S.dockBindWrap },
        h('button', {
          type: 'button',
          style: Object.assign({}, S.dockBindBtn, bound ? S.dockBindBtnOn : null),
          title: bound ? '已锁定：' + bindName + '（点击切换实例/解绑）' : '未锁定 Unity 实例：点击选择实例（新会话也可提前绑定）',
          onClick: function (e) { e.stopPropagation(); setPool(function (p) { return { ...p, open: !p.open } }) },
        },
          h('span', { style: Object.assign({ background: bound ? (bindAlive ? '#30a46c' : '#e5484d') : '#888' }, S.dot) }),
          h('span', null, bound ? bindName : '绑定 Unity'),
          h('span', { style: S.caret }, '▾')
        ),
        pool.open && h('div', { style: S.dockPop, onClick: function (e) { e.stopPropagation() } },
          renderDockPool(pool, bound, dockBind, dockUnbind, dockScan, function () { setPool(function (p) { return { ...p, open: false } }) })
        )
      )
      ...
      return h('div', { style: S.dockWrap, title: 'Unity 状态携带：每次发出指令自动注入勾选项；当前' + (enabled ? '开' : '关') + '。' + cacheText + '。截图落盘本地，正文只带文件路径' },
        ...
        h('span', { style: S.dockLbl },
          h('span', null, 'Unity'),
          h('label', { style: S.toggleWrap, title: bound ? (enabled ? '总开关：每次指令携带状态（开）' : '总开关：不携带状态（关）') : '未绑定 Unity，绑定后生效（开关按会话记忆）' },
            h('input', { type: 'checkbox', checked: enabled, onChange: function (e) { switchIt('stateEnabled', e.target.checked) }, style: S.toggleInput }),
            h('span', { style: Object.assign({}, S.toggleTrack, enabled ? S.toggleTrackOn : null) }),
            h('span', { style: Object.assign({}, S.toggleKnob, enabled ? { left: 16 } : null) })
          ),
          h('span', { style: bound ? null : { opacity: 0.7 } }, bound ? '状态携带' : '未绑定 Unity，绑定后生效')
        ),
        chipCells
      )
    }

    /** v0.5.3：dock 绑定浮窗内容（服务/实例列表 + 锁定/解绑/扫描）。 */
    function renderDockPool(pool, bound, onBind, onUnbind, onScan, onClose) {
      var view = pool.view
      var rows = []
      if (view && Array.isArray(view.services)) {
        view.services.forEach(function (s) {
          rows.push(h('div', { key: 'svc:' + s.id, style: S.svcHead },
          ...
          var insts = Array.isArray(s.instances) ? s.instances : []
          if (!insts.length) {
            rows.push(h('div', { key: 'svc:' + s.id + ':none', style: S.empty }, s.alive === false ? '服务离线' : '暂无实例（点「扫描」发现）'))
            return
          }
          insts.forEach(function (inst) {
            var active = inst.active === true
            rows.push(h('div', { key: s.id + ':' + inst.id, style: S.row },
              h('span', { style: Object.assign({ background: active ? '#30a46c' : s.alive === false ? '#e5484d' : '#888' }, S.dot) }),
              h('span', { style: S.nm, title: inst.id }, inst.name),
              h('span', { style: S.sub }, inst.hash),
              active && h('span', { style: Object.assign({}, S.badge, S.activeBadge) }, '本会话'),
              h('button', {
                style: active || pool.busy ? Object.assign({}, S.btn, S.btnDisabled) : S.btn,
                disabled: active || pool.busy,
                onClick: function () { onBind(inst.id) },
              }, active ? '已锁定' : '锁定')
            ))
          })
        })
      }
      if (!rows.length) rows.push(h('div', { key: 'loading', style: S.empty }, pool.busy ? '处理中…' : '读取服务池…'))
      return h('div', null,
        h('div', { style: S.head },
          h('span', { style: S.ttl }, 'Unity 服务池（本会话）'),
          h('button', { style: S.close, onClick: onClose, title: '关闭' }, '✕')
        ),
        h('div', { style: S.sub1 }, bound ? '并行开发同一实例属正常用法，直接锁定即可' : '未锁定实例（新会话也可提前绑定；或用 unity_pool_bind 工具）'),
        rows,
        pool.error && h('div', { style: S.err }, pool.error),
        h('div', { style: S.actions },
          bound && h('button', { style: pool.busy ? Object.assign({}, S.btn, S.btnDisabled) : S.btn, disabled: pool.busy, onClick: onUnbind }, '解绑'),
          h('button', { style: pool.busy ? Object.assign({}, S.btn, S.btnDisabled) : S.btn, disabled: pool.busy, onClick: onScan }, '扫描')
        )
      )
    }
